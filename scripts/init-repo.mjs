...
import { resolvePrimarySourcePath } from "./primary-source.mjs";
...
function listConcepts(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    const primaryPath = resolvePrimarySourcePath(pkgDir, entry.name);
    let fields = {};
    if (primaryPath && existsSync(primaryPath)) {
      fields = parseFrontmatterFields(readFileSync(primaryPath, "utf8"));
    } else {
